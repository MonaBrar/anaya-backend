...
async def store_lesson(lesson: Lesson):
    response = openai_client.embeddings.create(  # ✅ Fixed here
...
async def retrieve_lesson(query: str):
    ...
    # ✅ Ensure correct format for Pinecone query
    results = index.query(
        queries=[query_embedding],  # ✅ Pinecone expects a list
        top_k=3,
...
async def all_lessons():
    results = index.query(
        queries=[[0.0] * 1536],  # ✅ Correct format
        top_k=100,
        include_metadata=True
    )
